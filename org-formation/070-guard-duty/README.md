...
- `AWS::GuardDuty::Detector` deployed to `SecurityCentralAccount` in every region listed in the `guardDutyRegions` parameter (see `org-formation/_parameters.yaml`). This is a subset of commercial regions — opt-in regions and newer regions are excluded. Expand the list as needed.
